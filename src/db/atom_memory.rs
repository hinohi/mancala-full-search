...
pub struct AtomMemoryDB<K, V> {
    div: usize,
    db: Vec<RwLock<HashMap<K, V>>>,
}

impl<K, V> DB<K, V> for AtomMemoryDB<K, V>
where
    K: Hash + Eq,
    V: Clone,
{
...
impl<K, V> AtomMemoryDB<K, V>
where
    K: Hash + Eq,
    V: Clone,
{
    pub fn new(div: usize) -> AtomMemoryDB<K, V> {
...
mod tests {
    ...
    fn multi() {
        ...
        let db = Arc::new(AtomMemoryDB::new(64));
        let mut w = Vec::new();
        for i in 0..10 {
            let db = db.clone();
